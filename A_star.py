...
def a_star(screen,maze, source,target, end_algorithm=True,start_paused = True):
    ...
    while running :
        ...
        if not found:
            ...
            for i_n,j_n in neighbours_pos:
                ...
                if  not out_of_limits(i_n,j_n,ROWS-1,COLS-1)    and   not_obstacle(maze, i_n,j_n) :
                    ...
                    if dist[i_n][j_n] > cur_dist + peso(maze[i_n][j_n])  + estimative((i_n,j_n),target):           # found a better path to position (i_n,j_n)
                        dist[i_n][j_n] = cur_dist + peso(maze[i_n][j_n]) + estimative((i_n,j_n),target)           # update distance
        ...
        if found:
            ...
            if end_algorithm or path == False: 
